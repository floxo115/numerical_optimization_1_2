...
from test_functions import test_func_1_torch, test_func_2_torch

np.set_printoptions(precision=15)

def test_cdf_grad_test_at_minimum():
    expected = test_func_1_torch.get_gradient(np.array([1., 1.]))
    actual = cdf_grad(test_func_1_torch.function, np.array([1., 1.]))
...
def test_cdf_grad_test_at_random_points():
    for i in range(100):
        ...
        actual = cdf_grad(test_func_1_torch.function, rand_inp)
...
def test_cdf_hessian_test_at_random_points():
    for i in range(10):
        ...
        actual = cdf_hessian(test_func_1_torch.function, rand_inp, epsilon=10 ** -7)
